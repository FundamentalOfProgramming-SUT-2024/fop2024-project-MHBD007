...
void score_table(){
    ...
    for(int i=0;i<num_users;i++){
        if( i == 0 || i==1 || i==2 ){
            ...
            attron(COLOR_PAIR(1));
            mvprintw(5+i*2,60,"%2d    %10s    %2d    %2d    %2d",i+1,Champions[i].name,Champions[i].sum_points,
                                                      Champions[i].sum_golds,Champions[i].num_games);
            attroff(COLOR_PAIR(1));
            }
            else {
            attron(COLOR_PAIR(5));
    ...
    attron(COLOR_PAIR(4));
    mvprintw(18,60,"Press any keys to exit this menu");
    attroff(COLOR_PAIR(4));

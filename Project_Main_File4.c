...
User_Data Champions [100];

void initializing_monsters2(){
    for(int i=0;i<15;i++){
        Monsters[i].x = 0;
        Monsters[i].y = 0;
        Monsters[i].health = 0;
...
void Saving_User_Data(User_Data user) {
    ...
    fprintf(file, "%s %s %s %d %d %d %d %d\n", user.name, user.password,user.email,user.game_situation,user.num_games,user.sum_points,user.sum_golds,user.experience); 
    fclose(file);
}
...
void uploading_Data (){
    ...
    for(int i=0;i<num_users;i++)
        fprintf(file, "%s %s %s %d %d %d %d %d\n", Users[i].name, Users[i].password,Users[i].email,Users[i].game_situation,Users[i].num_games,Users[i].sum_points,Users[i].sum_golds,Users[i].experience); 
    fclose(file);
}
...
void Loading_users_information(){
    ...
    num_users = 0;
    while (fgets(line, sizeof(line), file)) {
    char temp1[100];char temp2[100];char temp3[100];int game_situation;int num_game;int sum_points;int sum_golds;int experience;
        if (sscanf(line, "%s %s %s %d %d %d %d %d", temp1, temp2, temp3,&game_situation,&num_game,&sum_points,&sum_golds,&experience) == 8) {
            strcpy(Users[num_users].name,temp1);
            strcpy(Users[num_users].password,temp2);
            strcpy(Users[num_users].email,temp3);
            Users[num_users].game_situation = game_situation ;
            Users[num_users].num_games = num_game ;
            Users[num_users].sum_points  = sum_points ;
            Users[num_users].sum_golds = sum_golds ;
            Users[num_users].experience = experience ;
            num_users++;
        }
        else{
            break;
    ...
    while((ch = getch()) != 'q'){
        ...
        if(ch == '2'){
            ...
            if(n == 0){
                ...
                mvprintw(2,80,"The difficulty level is set !");
                attroff(COLOR_PAIR(30));
            }
            else{
                Hero_color = m+1;
                attron(COLOR_PAIR(30));
                mvprintw(8,80,"The color is set !");
                attroff(COLOR_PAIR(30)); 
            }
        }
        int i = 48 + n*40 ;
...
void sorting_filling_champion(){
    ...
    for (int i = 0; i < num_users - 1; i++) {
        for (int j = i + 1; j < num_users; j++) {
            if (Users[indices[j]].sum_points > Users[indices[i]].sum_points) {
                int temp = indices[i];
                indices[i] = indices[j];
                indices[j] = temp;
            }
...
void score_table(){
    ...
    sorting_filling_champion();
    attron(COLOR_PAIR(4));
    mvprintw(1,60,"SCORE TABLE");
    mvprintw(3,60,"Rank    Name    points    golds    games_played");
    attroff(COLOR_PAIR(4));
    ...
    for(int i=0;i<num_users;i++){
        if( i == 0 || i==1 || i==2 ){
            attron(COLOR_PAIR(1));
            mvaddstr(5+i*2,55,U_GOLDEN_COIN);
            attroff(COLOR_PAIR(1));
            attron(A_ITALIC);
            if(strcmp(Champions[i].name,Users[current_user_index].name) != 0){
            
            
            attron(COLOR_PAIR(1));
            mvprintw(5+i*2,60,"%3d    %10s    %3d    %3d    %3d",i+1,Champions[i].name,Champions[i].sum_points,
                                                      Champions[i].sum_points,Champions[i].num_games);
            attroff(COLOR_PAIR(1));
            }
            else {
            attron(COLOR_PAIR(5));
    ...
    for(int i=0;i<3;i++){
    attron(COLOR_PAIR(4));
    mvprintw(5+i*2,50,"Goat");
    attroff(COLOR_PAIR(4));
    }

    attron(COLOR_PAIR(4));
    mvprintw(26,60,"Press any keys to exit this menu");
    attroff(COLOR_PAIR(4));
...
void Save_Game() {
    char filepath[256];
    sprintf(filepath, "/home/mhbd/Desktop/Users/%s1.txt", Users[current_user_index].name);
    FILE *file = fopen(filepath, "w");
    // if (file == NULL) {
    ...
    // Hero:
    fprintf(file," %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %c %c %c\n",
        Hero.level_cpy,Hero.x, Hero.y, Hero.health, Hero.point, Hero.level, Hero.food,
...
void Save_Map(){
    char filepath[256];
        sprintf(filepath, "/home/mhbd/Desktop/Users/%s2.txt", Users[current_user_index].name);
        FILE *file = fopen(filepath, "w");
...
void Load_Game() {
    ...
    // Hero:
    fscanf(file, "%d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %c %c %c\n",
        &Hero.level_cpy,&Hero.x, &Hero.y, &Hero.health, &Hero.point, &Hero.level, &Hero.food,
        &Hero.hunger, &Hero.n_fd, &Hero.p_fd, &Hero.m_fd, &Hero.b_fd,
        &Hero.DAGGER_num, &Hero.MAGIC_WAND_num, &Hero.NORMAL_ARROW_num,
        &Hero.SWORD_num, &Hero.p_helth_num, &Hero.p_speed_num, &Hero.p_damage_num,
...
do{
    ...
    if(Hero.level != Hero.level_cpy){
        Hero.level_cpy = Hero.level ;
        exit++;
        break ;
    }
...
//    endwin();
    ...
    //Saving_User_Data(Users[current_user_index]);
}
...
do{
    ...
    clear();
    attron(COLOR_PAIR(4));
    mvprintw(2,60,"Press Q to get to the login menu");
    mvprintw(4,60,"Press any other keys to get back to the pre game menu");
    attroff(COLOR_PAIR(4));
    ch = getch();
    ...
    clear();
    attron(COLOR_PAIR(4));
    mvprintw(2,60,"Press Q to end the game");
    mvprintw(4,60,"Press any other keys to get back to the login menu");
    attroff(COLOR_PAIR(4));
    ch = getch();
